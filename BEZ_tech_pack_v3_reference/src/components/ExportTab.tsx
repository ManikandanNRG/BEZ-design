...
export default function ExportTab({ data }: ExportTabProps) {
  const handleExportPDF = async () => {
    try {
      ...
      const element = document.getElementById('tech-pack-print-container');
      if (!element) {
      ...
      for (let i = 0; i < pages.length; i++) {
         ...
         pdfDoc.addImage(pageImgData, 'JPEG', 0, 0, pdfWidth, pdfHeight);
      }
      
      pdfDoc.save(`TechPack_${data.styleNo || 'Export'}.pdf`);
  ...
  const handleExportPPT = async () => {
    try {
      ...
      slide1.addText(`Tech Pack: ${data.styleName || 'Untitled'}`, { x: 1, y: 2.5, w: '80%', h: 1, fontSize: 36, bold: true, color: '363636' });
      slide1.addText(`Brand: ${data.brand || '-'}`, { x: 1, y: 3.5, w: '80%', h: 0.5, fontSize: 20 });
      slide1.addText(`Style No: ${data.styleNo || '-'}`, { x: 1, y: 4, w: '80%', h: 0.5, fontSize: 18 });
      slide1.addText(`Date: ${data.date || '-'}`, { x: 1, y: 4.5, w: '80%', h: 0.5, fontSize: 16, color: '666666' });

      // Slide 2: General Details (Metadata)
      const slide2 = pptx.addSlide();
      slide2.addText('General Information', { x: 0.5, y: 0.5, w: '90%', h: 0.5, fontSize: 24, bold: true, border: [0,0,{pt: 1, color: '000000'},0] });
      
       const rows = [
        ['BRAND', data.brand || '-', 'CATEGORY', data.category || '-'],
        ['STYLE NO.', data.styleNo || '-', 'SIZE RANGE', data.sizeRange || '-'],
        ['STYLE NAME', data.styleName || '-', 'FIT', data.fit || '-'],
        ['DESCRIPTION', data.description || '-', 'FABRIC', data.fabric || '-'],
        ['SEASON', data.season || '-', 'MADE IN', data.madeIn || '-'],
      ];
      
      slide2.addTable(rows, { x: 0.5, y: 1.5, w: 9, editable: true, border: { type: 'solid', pt: 1, color: 'DFDFDF' }, margin: 10, fontSize: 12 });

      // Slide 3: Images & Sketches
      if (data.frontSketch || data.backSketch) {
        const slide3 = pptx.addSlide();
        slide3.addText('Artwork & Sketches', { x: 0.5, y: 0.5, w: '90%', h: 0.5, fontSize: 24, bold: true, border: [0,0,{pt: 1, color: '000000'},0] });
        
        let imgX = 1;
        if (data.frontSketch) {
          slide3.addImage({ data: data.frontSketch, x: imgX, y: 1.5, w: 3, h: 4, sizing: { type: 'contain' } });
          imgX += 4;
        }
        if (data.backSketch) {
          slide3.addImage({ data: data.backSketch, x: imgX, y: 1.5, w: 3, h: 4, sizing: { type: 'contain' } });
        }
      }

      // Slide 4: Measurements
      if (data.measurements && data.measurements.length > 0) {
        const slide4 = pptx.addSlide();
        slide4.addText('Measurements', { x: 0.5, y: 0.5, w: '90%', h: 0.5, fontSize: 24, bold: true, border: [0,0,{pt: 1, color: '000000'},0] });
        
        const measHeaders = ['SR', 'DESCRIPTION', 'TOL', 'S', 'M', 'L', 'XL', 'XXL'];
        const measRows = [measHeaders, ...data.measurements.map(m => [m.srNo || '', m.description || '', m.tol || '', m.s || '', m.m || '', m.l || '', m.xl || '', m.xxl || ''])];
        
        slide4.addTable(measRows, { x: 0.5, y: 1.5, w: 9, rowH: 0.3, fontSize: 10, border: { type: 'solid', pt: 1, color: 'DFDFDF' }, autoPage: true });
      }

      await pptx.writeFile({ fileName: `TechPack_${data.styleNo || 'Export'}.pptx` });
    } catch (err) {
      console.error('PPT Export Error:', err);
      alert('Failed to generate PPT display export.');
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between no-print mb-8 pb-4 border-b border-gray-200">
        <div>
          ...
          <p className="text-sm text-gray-500">Preview generated exactly like the required tabular PDF template. Hit export to save (Landscape by default).</p>
        </div>
        <div className="flex gap-2">
          <button
            onClick={handleExportPPT}
            className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-black bg-white border border-gray-300 rounded-md hover:bg-gray-50 transition-colors shadow-sm"
          >
            <FileDown size={16} />
            Export PPT
          </button>
      ...
      <div id="tech-pack-print-container" className="bg-gray-100 p-4 sm:p-8 space-y-8 pb-12 print:bg-white print:p-0 print:space-y-0 print:pb-0 overflow-x-auto">
