...
async function startServer() {
  ...
  app.post("/api/vision-to-specs", async (req, res) => {
    try {
      ...
      console.log(`Analyzing image with vision-to-specs`);

      const base64Data = imageBase64.replace(/^data:image\/(png|jpeg|jpg|webp);base64,/, "");
      
      const prompt = `Analyze this garment sketch or mockup. Extract the following information and return ONLY a valid JSON object.
Do not wrap it in markdown block quotes. The format MUST be exactly:
{
  "materialComposition": "string",
  "fabricWeight": "string",
  "description": "brief description of the garment",
  "category": "garment category (e.g. T-Shirt, Pants, Dress)",
  "trimsAndHardware": "string",
  "labels": "string",
  "measurements": [
    {
      "description": "string (e.g. Chest Width, Body Length, Sleeve Length)",
      "tol": "string (e.g. +/- 0.5)",
      "s": "string",
      "m": "string",
      "l": "string",
...
IMPORTANT: DO NOT return any base64 image data or hallucinative long strings inside the JSON. Only return the requested text and numeric values. If something is unclear, make a reasonable professional estimation based on standard industry specs for this type of garment. Ensure grading is applied logically to the measurements. Provide 4 to 8 measurement points.`;
      ...
      console.error("vision-to-specs error:", error);
