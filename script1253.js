...
const tools = lines.filter(l => l.includes('"step_index":1253') && l.includes('multi_replace'));
if (tools.length > 0) {
  const tc = JSON.parse(tools[0]).tool_calls[0];
  const chunks = JSON.parse(tc.args.ReplacementChunks);
  fs.writeFileSync('scratch_1253.txt', chunks[0].ReplacementContent);
  console.log('Written to scratch_1253.txt');
}
